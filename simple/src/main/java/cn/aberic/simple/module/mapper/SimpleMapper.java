...
public interface SimpleMapper {
    ...
    @Insert("insert into org (hash,org_name,tls,ca_tls,username,crypto_config_dir,channel_artifacts_dir,ca_name,ca_location,org_msp_id,org_domain_name,orderer_domain_name," +
            "channel_name,chaincode_name,chaincode_source,chaincode_path,chaincode_version,proposal_wait_time,invoke_wait_time) " +
            "values (#{o.hash},#{o.orgName},#{o.tls},#{o.caTls},#{o.username},#{o.cryptoConfigDir},#{o.channelArtifactsDir},#{o.caName},#{o.caLocation},#{o.orgMSPID},#{o.orgDomainName}," +
    ...
    @Select("select hash,org_name,tls,ca_tls,username,crypto_config_dir,channel_artifacts_dir,ca_name,ca_location,org_msp_id,org_domain_name,orderer_domain_name," +
            "channel_name,chaincode_name,chaincode_source,chaincode_path,chaincode_version,proposal_wait_time,invoke_wait_time from org where hash=#{hash}")
    @Results({
    ...
    @Select("select hash,org_name,tls,ca_tls,username,crypto_config_dir,channel_artifacts_dir,ca_name,ca_location,org_msp_id,org_domain_name,orderer_domain_name," +
            "channel_name,chaincode_name,chaincode_source,chaincode_path,chaincode_version,proposal_wait_time,invoke_wait_time from org")
    @Results({
            @Result(property = "hash", column = "hash"),
            @Result(property = "orgName", column = "org_name"),
            @Result(property = "tls", column = "tls"),
